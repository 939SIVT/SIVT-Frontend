...
```mermaid
erDiagram
    users ||--o{ session_participants : "參與"
    chat_sessions ||--o{ session_participants : "包含參與者"
    chat_sessions ||--o{ chat_messages : "具有訊息"
    users ||--o{ chat_messages : "發送"
    file_types ||--o{ source_files : "分類"
    source_files ||--o{ message_attachments : "被附件"
    chat_messages ||--o{ message_attachments : "包含附件"
    source_files ||--o{ chat_messages : "被參考"
    ...
    file_types {
        int type_id PK
        string type_name
        string mime_type_pattern
        string processing_engine
    }
...
1. **Real_DB (關聯式)**:
   - **工具**: SQLite / PostgreSQL。
   - **用途**: 管理使用者 Session、多對多參與者關係、檔案中繼資料與稽核日誌 (Audit Logs)。
2. **RAG_DB (向量式)**:
   - **工具**: Milvus / FAISS。
   - **用途**: 儲存影片逐字稿與關鍵畫面的多模態嵌入向量，支援語意搜尋與跨平台索引。
3. **知識圖譜_DB (圖形化)**:
   - **工具**: Neo4j。
   - **用途**: 定義學科知識點 (Entity) 之間的邏輯關係（如：光折射 -> 凸透鏡 -> 成像原理），輔助 AI 生成更具結構性的引導問題與知識追蹤。
